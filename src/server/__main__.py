...
from lib.sql import Goal, SuperAgent, User, create_engine_from_env, ThreadItem
...
async def sms_reply():
    ...
    with _Session() as session:
        ...
        user_agent = UserAgent(super_agent=user.primary_agent, session=session, config=CONFIG)
...
if __name__ == "__main__":
    ...
    with _Session() as session:
        user = User.get_from_phone_number(session=session, phone_number=user_phone_number)
        if user is None:
            user = User(name="admin", phone_number=user_phone_number)
            super_agent = SuperAgent(name="admin", user=user)
            goal = Goal(super_agent=super_agent, objective="Ask the user what they want of you.")
